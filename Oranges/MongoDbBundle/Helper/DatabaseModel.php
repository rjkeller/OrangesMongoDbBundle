	...
	implements \ArrayAccess
{
	...
	*/
	...
	/**
	 ...
	 @param $forceOldColumn - The column must already exist for this change to
	   take place.
	*/
	public function setColumn($set, $val, $forceOldColumn = false)
	{
	    if (($forceOldColumn && !isset($this->__dbValues[$set])) ||
	        ($forceOldColumn && ($set == "id" || $set == "_id")))
	    {
	        return false;
        }
	...
	*/
	...
	{
		return $this->loadQuery(array("_id" => $id), true);
	}

	/**
	 Load the row with the specified query parameters from this table for
	 modification.
	 */
	...
	{
	    $mongoDbEvents = null;
	    if (MasterContainer::hasParameter("MongoDbModel"))
            $mongoDbEvents = MasterContainer::getParameter("MongoDbModel");

		//run events
		if ($mongoDbEvents != null &&
			isset($mongoDbEvents['ModelEvent']))
		{
			foreach ($mongoDbEvents['ModelEvent'] as $key => $value)
			{
		...
		if ($mongoDbEvents != null &&
			isset($mongoDbEvents['DbCache']))
		{
			foreach ($mongoDbEvents['DbCache'] as $key => $value)
			{
				if ($value['Model'] == get_class($this))
				{
					$cls = $value['Class'];
